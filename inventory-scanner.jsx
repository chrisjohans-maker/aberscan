  ...
  "categories": [
    {
      ...
      "items": [
        ...
        {"product":"Modelo Especial","type":"bottle","count":8,"details":"12oz"}
      ]
    },
    {
      "category": "Spirits",
...
Empty: {"categories":[],"total":0,"notes":"No inventory items detected"}`;
    ...
    try {
      ...
      if (isAuto) {
        setAggregated(prev => {
          ...
          setResults({ categories: merged, total: merged.reduce((s,g) => s + g.subtotal, 0), notes: parsed.notes });
          return merged;
        });
        setAutoCount(c => c + 1);
      } else {
        const wSub = cats.map(g => ({ ...g, subtotal: (g.items||[]).reduce((s,i) => s + i.count, 0) }));
        setResults({ categories: wSub, total: parsed.total, notes: parsed.notes });
      }
      setTab("results");
    ...
    <div style={S.root}>
      ...
      <div style={S.header}>
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontFamily: "'Playfair Display',serif", fontWeight: 800, color: "#FFF", border: "2px solid rgba(255,255,255,0.15)" }}>A</div>
        ...
        <button onClick={() => { if(typeof caches!=='undefined') caches.keys().then(k=>Promise.all(k.map(c=>caches.delete(c)))); window.location.reload(true); }} style={{ display:"flex", alignItems:"center", gap:5, background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:8, padding:"5px 10px", cursor:"pointer", color:"var(--muted,#8B7B72)", minHeight:32 }}>
          <span style={{ fontFamily:"var(--mono)", fontSize:9, letterSpacing:0.5 }}>v1.0.1</span>
      ...
      {showReport && results && (() => {
        ...
        (results.categories || []).forEach(g => {
          (g.items || []).forEach(item => {
            ...
            const par = pars[pk];
            const fresh = parseFreshness({ ...item, _cat: g.category });
            if (par && item.count < par) reorderItems.push({ ...item, _cat: g.category, par, need: par - item.count });
            if (fresh.status !== "ok") freshIssues.push({ ...item, _cat: g.category, fresh });
          });
        });

        const R = {
          overlay: { position:"fixed", inset:0, zIndex:100, background:"#FAFAF8", overflowY:"auto", WebkitOverflowScrolling:"touch" },
          page: { maxWidth:480, margin:"0 auto", padding:"20px 16px 40px", fontFamily:"'DM Sans',sans-serif", color:"#1A1A1A" },
          topBar: { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 },
          closeBtn: { background:"#F0EEEA", border:"none", borderRadius:8, padding:"8px 14px", fontFamily:"'DM Sans'", fontSize:13, fontWeight:600, color:"#555", cursor:"pointer", minHeight:36 },
          shareBtn: { background:"#8B2942", border:"none", borderRadius:8, padding:"8px 16px", fontFamily:"'DM Sans'", fontSize:13, fontWeight:700, color:"#FFF", cursor:"pointer", minHeight:36 },
          header: { textAlign:"center", marginBottom:24, paddingBottom:20, borderBottom:"1px solid #E8E5E0" },
          logo: { fontFamily:"'Playfair Display',serif", fontSize:22, fontWeight:800, color:"#1A1A1A", marginBottom:2 },
          logoAccent: { fontFamily:"'Space Mono',monospace", fontSize:18, fontWeight:700, color:"#8B2942" },
          subtitle: { fontFamily:"'Space Mono',monospace", fontSize:10, color:"#999", textTransform:"uppercase", letterSpacing:2.5, marginTop:6 },
          dateBlock: { fontFamily:"'DM Sans'", fontSize:13, color:"#666", marginTop:4 },
          statsRow: { display:"flex", gap:8, marginBottom:20 },
          statBox: { flex:1, background:"#F5F3F0", borderRadius:10, padding:"12px 10px", textAlign:"center" },
          statNum: { fontFamily:"'Space Mono',monospace", fontSize:24, fontWeight:700, display:"block" },
          statLabel: { fontFamily:"'Space Mono',monospace", fontSize:9, color:"#888", textTransform:"uppercase", letterSpacing:1.5, marginTop:2, display:"block" },
          alertBox: { background:"#FFF5F5", border:"1px solid #FED7D7", borderRadius:10, padding:"12px 14px", marginBottom:16 },
          alertTitle: { fontFamily:"'Space Mono',monospace", fontSize:10, fontWeight:700, color:"#C53030", textTransform:"uppercase", letterSpacing:1.5, marginBottom:8 },
          alertItem: { display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom:"1px solid #FEE2E2" },
          alertProd: { fontFamily:"'DM Sans'", fontSize:13, fontWeight:600, color:"#1A1A1A" },
          alertMeta: { fontFamily:"'Space Mono',monospace", fontSize:11, color:"#C53030", fontWeight:700 },
          warnBox: { background:"#FFFFF0", border:"1px solid #FEFCBF", borderRadius:10, padding:"12px 14px", marginBottom:16 },
          warnTitle: { fontFamily:"'Space Mono',monospace", fontSize:10, fontWeight:700, color:"#B7791F", textTransform:"uppercase", letterSpacing:1.5, marginBottom:8 },
          warnItem: { display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom:"1px solid #FEFCBF" },
          warnMeta: { fontFamily:"'Space Mono',monospace", fontSize:11, color:"#B7791F", fontWeight:700 },
          catBlock: { marginBottom:16 },
          catTitle: { display:"flex", alignItems:"center", gap:8, padding:"10px 0", borderBottom:"2px solid #E8E5E0", marginBottom:8 },
          catName: { fontFamily:"'DM Sans'", fontSize:15, fontWeight:700, color:"#1A1A1A" },
          catCount: { fontFamily:"'Space Mono',monospace", fontSize:12, fontWeight:700, color:"#888", background:"#F0EEEA", padding:"2px 8px", borderRadius:6 },
          row: { display:"flex", alignItems:"center", padding:"8px 0", borderBottom:"1px solid #F0EEEA", gap:8 },
          rowProduct: { flex:1, fontFamily:"'DM Sans'", fontSize:13, fontWeight:600, color:"#333" },
          rowType: { fontFamily:"'Space Mono',monospace", fontSize:9, color:"#888", background:"#F0EEEA", padding:"2px 6px", borderRadius:4, textTransform:"uppercase", fontWeight:700 },
          rowDetail: { fontFamily:"'DM Sans'", fontSize:11, color:"#999", flex:2 },
          rowCount: { fontFamily:"'Space Mono',monospace", fontSize:18, fontWeight:700, color:"#1A1A1A", minWidth:36, textAlign:"right" },
          footer: { textAlign:"center", padding:"20px 0", borderTop:"1px solid #E8E5E0", marginTop:20 },
          footerText: { fontFamily:"'Space Mono',monospace", fontSize:9, color:"#BBB", textTransform:"uppercase", letterSpacing:2 },
        };
        ...
        const handleShare = async () => {
          ...
          try {
            ...
            const file = new File([blob], `aberscan-report-${new Date().toISOString().split("T")[0]}.png`, { type: "image/png" });

            if (navigator.canShare && navigator.canShare({ files: [file] })) {
              await navigator.share({
                files: [file],
                title: `AberScan Report — ${dateStr}`,
              });
            } else if (navigator.share) {
              // fallback: share as text if file sharing not supported
              const text = `AberScan Report — ${dateStr}\n${totalItems} items scanned\n${reorderItems.length > 0 ? `⚠️ ${reorderItems.length} items need restock\n` : ""}${freshIssues.length > 0 ? `🕐 ${freshIssues.length} freshness issues\n` : ""}\n` +
                (results.categories || []).map(g =>
                  `${g.category} (${g.subtotal}):\n` + (g.items || []).map(i => `  • ${i.product} [${i.type}] — ${i.count}${i.details ? ` (${i.details})` : ""}`).join("\n")
                ).join("\n\n") +
                (reorderItems.length > 0 ? `\n\nNEED RESTOCK:\n` + reorderItems.map(i => `  • ${i.product} [${i.type}]: ${i.count} on hand, par ${i.par}, need ${i.need}`).join("\n") : "");
              await navigator.share({ title: `AberScan Report — ${dateStr}`, text });
            } else {
          ...
          finally { setSharingState(false); }
        };

        return (
          <div style={R.overlay}>
            <div style={R.page} ref={reportRef}>
              <div style={R.topBar}>
                <button style={R.closeBtn} onClick={() => setShowReport(false)}>← Back</button>
                <button style={{ ...R.shareBtn, opacity: sharing ? 0.6 : 1 }} onClick={handleShare} disabled={sharing}>
                  {sharing ? "Generating…" : "Share 📤"}
                </button>
              </div>

              {/* Everything below here gets captured as the image */}
              <div ref={captureRef} style={{ background: "#FAFAF8", padding: "4px 0 20px" }}>

              {/* Header */}
              <div style={R.header}>
                <div><span style={R.logo}>Aber</span><span style={R.logoAccent}>Scan</span></div>
                <div style={R.subtitle}>{showFullInventory ? "Full Inventory Report" : activeZone ? `${activeZone} Report` : "Inventory Report"}</div>
                <div style={R.dateBlock}>{dateStr} at {timeStr}</div>
                {Object.keys(zoneScans).length > 1 && showFullInventory && (
                  <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 6, marginTop: 10 }}>
                    {Object.entries(zoneScans).map(([z, zs]) => (
                      <span key={z} style={{ fontFamily: "'Space Mono',monospace", fontSize: 9, color: "#888", background: "#F0EEEA", padding: "3px 8px", borderRadius: 6 }}>{zoneIcon(z)} {z} ({zs.total})</span>
                    ))}
                  </div>
                )}
              </div>
              ...
              <div style={R.statsRow}>
                <div style={R.statBox}>
                  <span style={{ ...R.statNum, color: "#1A1A1A" }}>{totalItems}</span>
                  <span style={R.statLabel}>Total items</span>
                </div>
                <div style={R.statBox}>
                  <span style={{ ...R.statNum, color: (results.categories||[]).length > 0 ? "#1A1A1A" : "#CCC" }}>{(results.categories||[]).length}</span>
                  <span style={R.statLabel}>Categories</span>
                </div>
                <div style={R.statBox}>
                  <span style={{ ...R.statNum, color: reorderItems.length > 0 ? "#C53030" : "#2F855A" }}>{reorderItems.length}</span>
                  <span style={R.statLabel}>Low stock</span>
                </div>
                <div style={R.statBox}>
                  <span style={{ ...R.statNum, color: freshIssues.length > 0 ? "#B7791F" : "#2F855A" }}>{freshIssues.length}</span>
                  <span style={R.statLabel}>Fresh issues</span>
                </div>
              </div>

              {/* Reorder Alert */}
              {reorderItems.length > 0 && (
                <div style={R.alertBox}>
                  <div style={R.alertTitle}>🔴 Restock needed</div>
                  {reorderItems.map((item, i) => (
                    <div key={i} style={{ ...R.alertItem, borderBottom: i === reorderItems.length-1 ? "none" : R.alertItem.borderBottom }}>
                      <div>
                        <span style={R.alertProd}>{item.product}</span>
                        <span style={{ ...R.rowType, marginLeft:6 }}>{item.type}</span>
                      </div>
                      <span style={R.alertMeta}>Need {item.need} more</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {freshIssues.length > 0 && (
                <div style={R.warnBox}>
                  <div style={R.warnTitle}>⚠️ Freshness check</div>
                  {freshIssues.map((item, i) => (
                    <div key={i} style={{ ...R.warnItem, borderBottom: i === freshIssues.length-1 ? "none" : R.warnItem.borderBottom }}>
                      <div>
                        <span style={R.alertProd}>{item.product}</span>
                        <span style={{ ...R.rowType, marginLeft:6 }}>{item.type}</span>
                      </div>
                      <span style={R.warnMeta}>{item.fresh?.label || "check"}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {/* Category Inventory */}
              {(results.categories || []).map((g, gi) => (
                <div key={gi} style={R.catBlock}>
                  <div style={R.catTitle}>
                    <span style={{ fontSize: 16 }}>{catStyle(g.category).icon}</span>
                    <span style={R.catName}>{g.category}</span>
                    <span style={R.catCount}>{g.subtotal}</span>
                  </div>
                  {(g.items || []).map((item, ii) => {
                    const pk = parKey(item);
                    const par = pars[pk];
                    const isLow = par && item.count < par;
                    return (
                      <div key={ii} style={{ ...R.row, background: isLow ? "#FFF5F5" : "transparent" }}>
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}>
                            <span style={R.rowProduct}>{item.product}</span>
                            <span style={R.rowType}>{item.type}</span>
                            {par && <span style={{ fontFamily:"'Space Mono',monospace", fontSize:9, color:"#888" }}>par:{par}</span>}
                          </div>
                          {item.details && <div style={{ fontFamily:"'DM Sans'", fontSize:11, color:"#999", marginTop:2 }}>{item.details}</div>}
                        </div>
                        <span style={{ ...R.rowCount, color: isLow ? "#C53030" : "#1A1A1A" }}>{item.count}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              {results.notes && results.notes !== "No inventory items detected" && (
                <div style={{ background:"#F5F3F0", borderRadius:10, padding:"12px 14px", marginTop:12 }}>
                  ...
                  <div style={{ fontFamily:"'DM Sans'", fontSize:13, color:"#555", lineHeight:1.5 }}>{results.notes}</div>
                </div>
              )}

              <div style={R.footer}>
                <div style={R.footerText}>Generated by AberScan • The Aberdeen Tap</div>
              </div>
              </div>{/* end captureRef */}
            </div>
          </div>
